...
| Application configuration/secrets | Non-development `AddDefaultConfiguration` optionally connects to Azure App Configuration with `DefaultAzureCredential` and resolves Key Vault references; Shipping requires `common:Shipping:SignatureSecret` (null label) through that path | `APPCONFIGURATION` is optional globally, `KEYVAULT` is set locally but not read directly, and required-key validation is not yet centralized beyond adopted options such as Shipping |
...
| Webhook signatures | `Payment:SignatureSecret` remains hardcoded locally; Shipping now receives one required secret from an Aspire secret parameter in Development and a documented Azure App Configuration Key Vault reference in non-development; the Shipping simulator still sends literal `Signature` | Payment verification returns true solely for literal `Signature`; shipping does not yet validate incoming signatures, so sourcing the Shipping secret closes configuration exposure but not SEC-01 cryptographic verification |
